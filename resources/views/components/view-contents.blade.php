...
<div class="flex flex-col h-full" wire:poll.500ms>
    <div class="@if($item->color) bg-{{ $item->color->name }}-500 @else bg-primary-dark @endif rounded-t-lg text-white">
        <div class="flex @if($backRoute) justify-between @else justify-end @endif w-full px-10 pt-4">
            @if($backRoute)
                <x-button 
                        :link="$backRoute"
                        icon="fa-left-long fa-xl" 
                        buttonClasses="max-w-content hover:scale-110 slow-hover"
                        theme="none"
                    >
                    <p class="text-xs font-bold text-center">Back</p>
                </x-button>
            @endif
